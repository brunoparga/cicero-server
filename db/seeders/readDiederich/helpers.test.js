...
describe("treatDefinition", () => {
  ...
  it("lowercases a starting capital E", () => {
    ...
    expect(treatDefinition(definition)).toBe(
...
// A compare function used in sorting 2-element arrays alphabetically by
// the second element. Must return 1 if the arguments must be switched,
// -1 if not (and 0 if they're equal).
describe("compare", () => {
  const foo = ["foo", "quux"];
  ...
  it("correctly keeps already alphabetized words", () => {
    ...
    expect(compare(fred, xyzzy)).toEqual(alreadyInOrder);
  });
...
// A filtering function, returning true iff an element is either the first in
// the array or its head is different from the preceding one's head.
describe("deduplicate", () => {
  ...
  it("returns true if the head of the first element is different from the preceding one", () => {
    ...
    expect(deduplicate(["baz", "quux"], thirdElement, array)).toBe(true);
  });
...
describe("multiIncludes", () => {
  ...
  it("correctly returns false when no needles are in the haystack", () => {
    expect(multiIncludes(description, ["nail", "pin", "thumbtack"])).toBe(
      false
    );
  });
